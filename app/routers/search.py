...
def search_books(
    query_main: Optional[str] = Query(None, description="Повнотекстовий запит по title/author/genres"),
    ...
    if query_main:
        must.append({
            "multi_match": {
                "query": query_main,
                "fields": ["title^3", "author^2", "genres", "title.raw", "author.raw"]
            }
        })
    if genre:
    ...
    buckets = {b["key"]: b["avg_rating"]["value"] for b in rev["aggregations"]["by_book"]["buckets"]}
